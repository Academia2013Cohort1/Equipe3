...
private var AutoScrollSpeed:float = 0f;
private var shipToFollow:scr_VaisseauAvatar;
//private var shipPosCourante:Vector3;
//private var shipPosFramePrecedent:Vector3;
private var vecteurAutoScroll:Vector3;
...
private var springiness = 0;
//var nePeutPasReculer : boolean = false;
...
function AutoScroll(value:float){
	AutoScrollSpeed = value;
	shipToFollow.setAutoScroll(value);
	vecteurAutoScroll = Vector3(AutoScrollSpeed,0,0);
}

function SetShipToFollow(ship:scr_VaisseauAvatar){
...
function LateUpdate () {
	...
	//if(!nePeutPasReculer || (nePeutPasReculer && goalPosition.y<transform.position.y))
	transform.position = Vector3.Lerp (transform.position, goalPosition, Time.deltaTime * springiness);
//	else if(AutoScrollSpeed>0){
//		transform.position = Vector3.Lerp(transform.position, transform.position +vecteurAutoScroll, Time.deltaTime*springiness );
}
...
function GetGoalPosition () {
	...
	var goalPosition = target.position + Vector3 (0, heightOffset, -distance * distanceModifier)+vecteurAutoScroll;
